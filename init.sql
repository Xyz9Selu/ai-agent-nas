...
-- Placeholders: __RAG_POSTGRES_DB__, __RAG_POSTGRES_USER__, __RAG_POSTGRES_PASSWORD__, __VECTOR_SIZE__
...
SELECT 'CREATE DATABASE __RAG_POSTGRES_DB__' WHERE NOT EXISTS (SELECT 1 FROM pg_database WHERE datname = '__RAG_POSTGRES_DB__')\gexec

-- 3. Create user if not exists
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = '__RAG_POSTGRES_USER__') THEN
    EXECUTE format('CREATE USER %I WITH ENCRYPTED PASSWORD %L', '__RAG_POSTGRES_USER__', '__RAG_POSTGRES_PASSWORD__');
  END IF;
END $$;

-- 4. Grant all privileges on both databases
GRANT ALL PRIVILEGES ON DATABASE __RAG_POSTGRES_DB__ TO __RAG_POSTGRES_USER__;
GRANT ALL PRIVILEGES ON DATABASE __POSTGRES_DB__ TO __RAG_POSTGRES_USER__;
GRANT ALL PRIVILEGES ON DATABASE __RAG_POSTGRES_DB__ TO __RAG_POSTGRES_USER__;
...
GRANT ALL ON SCHEMA public TO __RAG_POSTGRES_USER__;
GRANT USAGE ON SCHEMA public TO __RAG_POSTGRES_USER__;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO __RAG_POSTGRES_USER__;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT USAGE, SELECT ON SEQUENCES TO __RAG_POSTGRES_USER__;

-- 7. Connect to ai_agent database for schema and extension setup
\connect __RAG_POSTGRES_DB__
...
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO __RAG_POSTGRES_USER__;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO __RAG_POSTGRES_USER__;
