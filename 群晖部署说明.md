...
**必填参数：**
- **数据库密码**：`POSTGRES_PASSWORD`、`RAG_POSTGRES_PASSWORD`（请设置强密码，n8n 数据库连接配置会自动从这些值派生）
...
> 💡 n8n 的 `GENERIC_TIMEZONE` 已在容器内部自动从 `TZ` 派生，无需手动配置。
...
| 变量名 | 建议值 | 说明 | 必填 |
|--------|--------|------|------|
| `RAG_POSTGRES_DB` | `ai_agent` | AI agent 数据库名 | ✅ |
| `RAG_POSTGRES_USER` | `n8nuser` | AI 数据库用户名 | ✅ |
| `RAG_POSTGRES_PASSWORD` | `设置强密码` | ⚠️ **必须修改**为强密码 | ✅ |
...
> 💡 **提示：** n8n 的数据库连接配置（`DB_TYPE`、`DB_POSTGRESDB_HOST`、`DB_POSTGRESDB_PORT`、`DB_POSTGRESDB_DATABASE`、`DB_POSTGRESDB_USER`、`DB_POSTGRESDB_PASSWORD`）已在容器内部自动设置，无需手动配置。这些值从 `POSTGRES_*` 变量自动派生。
...
> - n8n 数据库连接配置已自动从 `POSTGRES_*` 变量派生，无需额外配置
...
RAG_POSTGRES_DB=ai_agent
RAG_POSTGRES_USER=n8nuser
RAG_POSTGRES_PASSWORD=MySecurePass2024!
...
> 💡 n8n 数据库连接配置（`DB_POSTGRESDB_*`）已在容器启动时自动设置，无需在此配置。
...
- User: 环境变量中的 `RAG_POSTGRES_USER`
- Password: 环境变量中的 `RAG_POSTGRES_PASSWORD`
- SSL: 关闭
